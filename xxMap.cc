// $Id: xxMap.cc 2641 2007-09-02 21:31:02Z flaterco $
...
const bool xxMap::translateCoordinates (double lat,
  ...
  x_out = internalWidth + (int) (((lng - bounds[zoomLevel].wlon) /
          (bounds[zoomLevel].elon - bounds[zoomLevel].wlon)) * x_map_size);
  y_out = internalHeight + (int) (((bounds[zoomLevel].nlat - lat) /
          (bounds[zoomLevel].nlat - bounds[zoomLevel].slat)) * y_map_size);
  return true;
}
...
void xxMap::untranslateCoordinates (double &lat_out,
                                    ...
                                    int x,
                                    int y) {
  lng_out = bounds[zoomLevel].wlon + ((double) (x - internalWidth) /
            (double) (x_map_size - internalWidth)) * (bounds[zoomLevel].elon -
            bounds[zoomLevel].wlon);
  lat_out = bounds[zoomLevel].nlat - ((double) (y - internalHeight) /
            (double) (y_map_size - internalHeight)) * (bounds[zoomLevel].nlat -
            bounds[zoomLevel].slat);
}
...
void xxMap::redrawMap () {
  ...
  int startlat ((int) (bounds[zoomLevel].slat - 1.0));
  int endlat   ((int) (bounds[zoomLevel].nlat + 1.0));
  int startlon ((int) (bounds[zoomLevel].wlon - 1.0));
  int endlon   ((int) (bounds[zoomLevel].elon + 1.0));
  ...
  if (redrawZeroLevel || zoomLevel) {
	...
	nseg = Global::wvsrtv (files[coast], lat, lon, &latray, &lonray,
	                       &segray);
    ...
    Global::wvsrtv ("clean", 0, 0, &latray, &lonray, &segray);
  } else
    XCopyArea (xxX::display, savemap, mapPixmap, xxX::markGC,
  ...
  if (redrawZeroLevel) {
    savemap = xxX::makePixmap (x_map_size + 1, y_map_size + 1);
    XCopyArea (xxX::display, mapPixmap, savemap, xxX::markGC,
	       0, 0, x_map_size, y_map_size, 0, 0);
  ...
  for (unsigned long i = 0 ; i < stationIndex.size() ; ++i) {
	...
	XFillArc (xxX::display, mapPixmap, xxX::markGC,
  ...
  Arg args[1] = {{"bitmap", (XtArgVal)mapPixmap}};
...
void xxMap::blast (int x, int y) {
  ...
  for (unsigned long a = 0 ; a < stationIndex.size() ; ++a) {
    ...
    if (translateCoordinates (stationIndex[a]->coordinates, bx, by)) {
      ...
      if (sqrt (dx * dx + dy * dy) <= rightClickRadius)
...
void xxMap::keyboard (KeySym key) {
  ...
  double xsize ((bounds[zoomLevel].elon - bounds[zoomLevel].wlon));
  double ysize ((bounds[zoomLevel].nlat - bounds[zoomLevel].slat));
...
void xxMap::mouseButton (const XButtonEvent *xbe) {
  ...
  int rx (xbe->x - internalWidth);
  int ry (xbe->y - internalHeight);
  ...
  if (xbe->button == Button1) {
    if (zoomLevel < maxZoomLevel) {
      ...
      untranslateCoordinates (lat, lon, xbe->x, xbe->y);
  ...
  if (xbe->button == Button2) {
    ...
    if (closestsr)
      Global::root->newGraph (*closestsr);
  }
}


xxMap::xxMap (const xxWidget &shell):
  xxWindow (shell, boxContainer),
  ...
  };
  {
    ...
    Widget latitudeLabelWidget = XtCreateManagedWidget ("Lat -00.0000",
      labelWidgetClass, container->widget(), latargs, 2);
    ...
    Widget longitudeLabelWidget = XtCreateManagedWidget ("Lon -000.0000",
      labelWidgetClass, container->widget(), lonargs, 2);
